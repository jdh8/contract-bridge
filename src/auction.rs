//! Auction lingo: calls, legal-call enforcement, and the sequence of calls
//! that make up the bidding phase of a deal.
//!
//! [`Call`] is the exhaustive set of legal announcements during the bidding —
//! pass, double, redouble, or a [`Bid`]. [`Auction`] stores a sequence of
//! `Call`s and enforces the laws of duplicate bridge on each insertion,
//! reporting violations via [`IllegalCall`].
//!
//! [`RelativeVulnerability`] is a 2-bit set (WE / THEY) for vulnerability
...
impl Auction {
    ...
    /// absolute [`Seat`](crate::Seat), add the dealer's seat offset modulo 4.
    ///
    ...
    ///
    /// ```
    /// use contract_bridge::auction::{Auction, Call, IllegalCall};
