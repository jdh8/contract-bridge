//! Random card shuffling and deal generation.
//!
//! [`Deck`] is a 52-card-bounded set backed by [`Hand`]; cards can be inserted
//! and drawn (uniformly at random) without materializing the full card set.
//! [`full_deal`] shuffles all 52 cards into a [`FullDeal`], and [`fill_deals`]
//! returns an infinite iterator that completes a [`PartialDeal`] with random
//! cards on each iteration.
//!
//! This module is gated behind the `rand` feature.

use crate::{Builder, Card, FullDeal, Hand, PartialDeal, Seat};
use core::fmt;
