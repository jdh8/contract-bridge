...
fn card() -> impl Strategy<Value = Card> {
    (0usize..contract_bridge::Suit::ASC.len(), rank()).prop_map(|(i, rank)| Card {
        suit: contract_bridge::Suit::ASC[i],
        rank,
    })
}
...
fn hand() -> impl Strategy<Value = Hand> {
    [holding(), holding(), holding(), holding()].prop_map(|[c, d, h, s]| {
        ...
        hand[contract_bridge::Suit::Clubs] = c;
        hand[contract_bridge::Suit::Diamonds] = d;
        hand[contract_bridge::Suit::Hearts] = h;
        hand[contract_bridge::Suit::Spades] = s;
        hand
    })
}

fn all_cards() -> impl Iterator<Item = Card> {
    contract_bridge::Suit::ASC.into_iter().flat_map(|suit| {
